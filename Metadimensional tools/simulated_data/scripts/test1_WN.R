...
# simulating case A with noise small dataset
...
for (i in 1:500){
  ...
  # group3
  geSimulated[41:60,i] <- rnorm(n = 20, mean = gmean+(gmean/2), sd = gsd+(gsd/10)) + rnorm(n=20, mean =0, sd = 0.4)
}
for (i in 501:600){
  gmean <- geMeans[ind[i]]
...
save(geSimulated, mirnaSimulated, methSimulated, sample_labels, file= "SimulatedCaseA_WN.RData")
