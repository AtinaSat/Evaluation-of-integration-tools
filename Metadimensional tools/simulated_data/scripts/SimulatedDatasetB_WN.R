...
# simulating case B with noise small dataset
...
geSimulated <- matrix(,nrow = 60, ncol = 600)
...
for (i in 501:600){
  gmean <- geMeans[ind[i]]
  geSimulated[1:60,i] <- rnorm(n=60, mean = gmean, sd = 2)
}
...
mirnaSimulated <- matrix(,nrow = 60, ncol = 520)
...
for (i in 1:500){
  ...
  mirnaSimulated[1:20,i] <- rnorm(n = 20, mean = mus[components], sd = sds[components]) + rnorm(n=20, mean =0, sd = 0.4)
  # group2
  mirnaSimulated[21:40,i] <- rnorm(n = 20, mean = mirnamean-(mirnamean/2), sd = mirnasd) + rnorm(n=20, mean =0, sd = 0.4)
  # group3
  mirnaSimulated[41:60,i] <- rnorm(n = 20, mean = mirnamean+(mirnamean/2), sd = mirnasd+(mirnasd/10)) + rnorm(n=20, mean =0, sd = 0.4)
}
for (i in 501:520){
  mirnamean <- miMeans[ind[i]]
  mirnaSimulated[1:60,i] <- rnorm(n=60, mean = mirnamean, sd = 2)
}
...
methSimulated <- matrix(,nrow = 60, ncol = 520)

for (i in 1:500){
  methmean <- meMeans[ind[i]]
  methsd <- meSds[ind[i]]
  # group1
  methSimulated[1:20,i] <- rnorm(n = 20, mean = methmean, sd = methsd) + rnorm(n=20, mean =0, sd = 0.4)
  # group2
...
for (i in 501:520){
  methmean <- meMeans[ind[i]]
  methSimulated[1:60,i] <- rnorm(n=60, mean = methmean, sd = 2)
}
...
save(geSimulated, mirnaSimulated, methSimulated, sample_labels, file= "SimulatedCaseB_WN.RData")
