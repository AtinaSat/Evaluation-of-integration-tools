...
# simulating case C with noise large dataset
# Group3 of Data type 1 and Group2 of Data type 2 and Group2 of Data type 3 not distinguishable
...
## for mixed signal
set.seed(999)
components <- sample(1:2,prob = c(0.5,0.5), size = 50, replace = T)

## for pca
source("/home/anita/utilities/pca_cor/PC-corr_net-master/R version/PC_corr_v2.R")
feat_names_ge <- c(paste0("Gene",as.character(1:600)))
feat_names <- c(paste0("Gene", as.character(1:520)))
sample_labels <- c(rep("Group1",50), rep("Group2",50), rep("Group3",50))
sample_names <- c(paste0("S",as.character(1:150)))
...
set.seed(999)
ind <- sample(1:1000, 600)
geSimulated <- matrix(,nrow = 150, ncol = 600)

for (i in 1:500){
  gmean <- geMeans[ind[i]]
  gsd <- geSds[ind[i]]
  # group1
  geSimulated[1:50,i] <- rnorm(n = 50, mean = gmean, sd = gsd) + rnorm(n=50, mean =0, sd = 0.4)
  # group2
  geSimulated[51:100,i] <- rnorm(n = 50, mean = gmean-(gmean/2), sd = gsd) + rnorm(n=50, mean =0, sd = 0.4)
  # group3
  mus = c(gmean, gmean-gmean/2)
  sds = c(gsd,gsd)
  geSimulated[101:150,i] <- rnorm(n = 50, mean = mus[components], sd = sds[components]) + rnorm(n=50, mean =0, sd = 0.4)
}
for (i in 501:600){
  gmean <- geMeans[ind[i]]
  geSimulated[1:150,i] <- rnorm(n=150, mean = gmean, sd = 2)
}

#PC_corr_v2(geSimulated,sample_labels,feat_names_ge, sample_names)
...
set.seed(444)
ind <- sample(1:dim(mirna)[2], 520)

mirnaSimulated <- matrix(,nrow = 150, ncol = 520)

for (i in 1:500){
  mirnamean <- miMeans[ind[i]]
  mirnasd <- miSds[ind[i]]
  # group1
  mirnaSimulated[1:50,i] <- rnorm(n = 50, mean = mirnamean, sd = mirnasd) + rnorm(n=50, mean =0, sd = 0.4)
  # group2
  mus = c(mirnamean, mirnamean+mirnamean/2)
  sds = c(mirnasd, mirnasd+(mirnasd/10))
  mirnaSimulated[51:100,i] <- rnorm(n = 50, mean = mus[components], sd = sds[components]) + rnorm(n=50, mean =0, sd = 0.4)
  # group3
  mirnaSimulated[101:150,i] <- rnorm(n = 50, mean = mirnamean+(mirnamean/2), sd = mirnasd+(mirnasd/10)) + rnorm(n=50, mean =0, sd = 0.4)
}
for (i in 501:520){
  mirnamean <- miMeans[ind[i]]
  mirnaSimulated[1:150,i] <- rnorm(n=150, mean = mirnamean, sd = 2)
}
...
set.seed(555)
ind <- sample(1:dim(meth)[2], 520, replace = T)
methSimulated <- matrix(,nrow = 150, ncol = 520)

for (i in 1:500){
  methmean <- meMeans[ind[i]]
  methsd <- meSds[ind[i]]
  # group1
  methSimulated[1:50,i] <- rnorm(n = 50, mean = methmean, sd = methsd) + rnorm(n=50, mean =0, sd = 0.4)
  # group2
  mus = c(methmean, methmean+methmean/2)
  sds = c(methsd, methsd+(methsd/10))
  methSimulated[51:100,i] <- rnorm(n = 50, mean = mus[components], sd = sds[components]) + rnorm(n=50, mean =0, sd = 0.4)
  # group3
  methSimulated[101:150,i] <- rnorm(n = 50, mean = methmean+(methmean/2), sd = methsd+(methsd/10)) + rnorm(n=50, mean =0, sd = 0.4)
}
for (i in 501:520){
  methmean <- meMeans[ind[i]]
  methSimulated[1:150,i] <- rnorm(n=150, mean = methmean, sd = 2)
}
...
save(geSimulated, mirnaSimulated, methSimulated, sample_labels, file= "SimulatedCaseC_WN_150Samples.RData")
