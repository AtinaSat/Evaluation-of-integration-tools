...
# simulating case A - with noise large dataset
# all groups clearly distinguishable
...
for (i in 1:500){
  ...
  # group3
  geSimulated[101:150,i] <- rnorm(n = 50, mean = gmean+(gmean/2), sd = gsd+(gsd/10)) + rnorm(n=50, mean =0, sd = 0.4)
}
for (i in 501:600){
  gmean <- geMeans[ind[i]]
...
# non-centered z-score

colnames(methSimulated) <- feat_names
rownames(methSimulated) <- sample_names

setwd("/home/anita/Benchmarking/simulated_data/")
save(geSimulated, mirnaSimulated, methSimulated, sample_labels, file= "SimulatedCaseA_WN_150Samples.RData")
