...
## Choosing 500 genes with most variance as we want a small set to work with
mirna.Var <- apply(mirna, 2, var)
top.mirna.Var <- sort(mirna.Var, decreasing = TRUE)[1:500]
mirna <- mirna[, attr(top.mirna.Var, "names")]

## Removing genes with near zero or zero variance
rm.cols <- nearZeroVar(dat.mrna, names = T)
all.cols <- colnames(dat.mrna)
mrna <- dat.mrna[,setdiff(all.cols, rm.cols)]
## Choosing 1000 genes with most variance as we want a small set to work with
mrna.Var <- apply(mrna, 2, var)
top.mrna.Var <- sort(mrna.Var, decreasing = T)[1:1000]
mrna <- mrna[,attr(top.mrna.Var, "names")]
...
## Choosing 1000 probes with most variance as we want a small set to work with
me.Var <- apply(me, 2, var)
top.me <- sort(me.Var, decreasing = T)[1:1000]
me <- me[,attr(top.me, "names")]
