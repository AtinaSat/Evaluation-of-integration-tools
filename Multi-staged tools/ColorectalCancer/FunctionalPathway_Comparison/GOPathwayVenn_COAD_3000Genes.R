...
cancerBP <- read.delim("DAVID/CancerGenes_GOTERM_BP_FAT.txt",
                       header = T, sep = "\t")
cancerBP <- cancerBP[cancerBP$FDR<0.05,]
cancerBP <- as.character(cancerBP$Term)

# CNAmet genes BP terms
CNAmetBP <- read.delim("ColorectalCancer/CNAmet/DAVID/CNAmet_COAD_3000Genes_GOTERM_BP_FAT.txt",
                       header = T, sep = "\t")
CNAmetBP <- CNAmetBP[CNAmetBP$FDR<0.05,]
CNAmetBP <- as.character(CNAmetBP$Term)

#iGC genes BP terms
iGCBP <- read.delim("ColorectalCancer/iGC/DAVID/iGC_COAD_3000Genes_GOTERM_BP_FAT.txt",
                    header = T, sep = "\t")
iGCBP <- iGCBP[iGCBP$FDR<0.05,]
iGCBP <- as.character(iGCBP$Term)

# PLRS genes BP terms
plrsBP <- read.delim("ColorectalCancer/PLRS/DAVID/PLRS_COAD_3000Genes_GOTERM_BP_FAT.txt",
                     header = T, sep = "\t")
plrsBP <- plrsBP[plrsBP$FDR<0.05,]
plrsBP <- as.character(plrsBP$Term)

# Oncodrive-CIS genes BP terms
OncoBP <- read.delim("ColorectalCancer/OncodriveCIS/DAVID/Oncodrive_COAD_3000Genes_GOTERM_BP_FAT.txt",
                     header = T, sep = "\t")
...
# Cancer genes
cancerCC <- read.delim("DAVID/CancerGenes_GOTERM_CC_FAT.txt",
                       header = T, sep = "\t")
cancerCC <- cancerCC[cancerCC$FDR<0.05,]
cancerCC <- as.character(cancerCC$Term)

# CNAmet
CNAmetCC <- read.delim("ColorectalCancer/CNAmet/DAVID/CNAmet_COAD_3000Genes_GOTERM_CC_FAT.txt",
                       header = T, sep = "\t")
CNAmetCC <- CNAmetCC[CNAmetCC$FDR<0.05,]
CNAmetCC <- as.character(CNAmetCC$Term)

# iGC
iGCCC <- read.delim("ColorectalCancer/iGC/DAVID/iGC_COAD_3000Genes_GOTERM_CC_FAT.txt",
                    header = T, sep = "\t")
iGCCC <- iGCCC[iGCCC$FDR<0.05,]
iGCCC <- as.character(iGCCC$Term)

# PLRS
plrsCC <- read.delim("ColorectalCancer/PLRS/DAVID/PLRS_COAD_3000Genes_GOTERM_CC_FAT.txt",
                     header = T, sep = "\t")
plrsCC <- plrsCC[plrsCC$FDR<0.05,]
plrsCC <- as.character(plrsCC$Term)

# Oncodrive-CIS
OncoCC <- read.delim("ColorectalCancer/OncodriveCIS/DAVID/Oncodrive_COAD_3000Genes_GOTERM_CC_FAT.txt",
                     header = T, sep = "\t")
...
# cancer
cancerMF <- read.delim("DAVID/CancerGenes_GOTERM_MF_FAT.txt",
                       header = T, sep = "\t")
cancerMF <- cancerMF[cancerMF$FDR<0.05,]
cancerMF <- as.character(cancerMF$Term)

# CNAmet
CNAmetMF <- read.delim("ColorectalCancer/CNAmet/DAVID/CNAmet_COAD_3000Genes_GOTERM_MF_FAT.txt",
                       header = T, sep = "\t")
CNAmetMF <- CNAmetMF[CNAmetMF$FDR<0.05,]
CNAmetMF <- as.character(CNAmetMF$Term)

# iGC
iGCMF <- read.delim("ColorectalCancer/iGC/DAVID/iGC_COAD_3000Genes_GOTERM_MF_FAT.txt",
                    header = T, sep = "\t")
iGCMF <- iGCMF[iGCMF$FDR<0.05,]
iGCMF <- as.character(iGCMF$Term)

# PLRS
plrsMF <- read.delim("ColorectalCancer/PLRS/DAVID/PLRS_COAD_3000Genes_GOTERM_MF_FAT.txt",
                     header = T, sep = "\t")
plrsMF <- plrsMF[plrsMF$FDR<0.05,]
plrsMF <- as.character(plrsMF$Term)

# Oncodrive-CIS
OncoMF <- read.delim("ColorectalCancer/OncodriveCIS/DAVID/Oncodrive_COAD_3000Genes_GOTERM_MF_FAT.txt",
                     header = T, sep = "\t")
