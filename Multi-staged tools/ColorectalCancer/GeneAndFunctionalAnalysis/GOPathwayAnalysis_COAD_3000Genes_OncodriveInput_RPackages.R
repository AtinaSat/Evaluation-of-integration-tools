...
COAD_FunctionalAnalysis <- data.frame(matrix(ncol = 17, nrow = 6))
colnames(COAD_FunctionalAnalysis) <- c("Tools", "Omics", "Sig.Genes", "Sig.Genes in CCGL", "Sig.Genes in CGC",
...
COAD_FunctionalAnalysis$Tools <- c("CNAMet", "iGC", "PLRS", "Oncodrive-CIS", "CNAMet", "MethylMix")
COAD_FunctionalAnalysis$Omics <- c(rep("CNA+GE", 4), rep("ME+GE", 2))

## CNAmet
# Reading the file
setwd("/home/anita/Benchmarking/two_omics/ColonCancerCompleteDataAnalysis/")
CNAmetTotal <- read.delim("/home/anita/Benchmarking/two_omics/ColonCancerCompleteDataAnalysis/CNAmet/CNAmet_UsingOncodriveCISInput/CNAmet_COAD_3000Genes_OncodriveInput.txt",
                          header = T, sep = "\t") 
...
COAD_FunctionalAnalysis[1, c(3, 6, 9, 12, 15)] <- c(length(CNAmetTotal), nrow(CNAmetBP), nrow(CNAmetCC), nrow(CNAmetMF), length(CNAmetPathway)) # Significant genes
COAD_FunctionalAnalysis[1, c(4, 7, 10, 13, 16)] <- EstimateCCGLOverlap(Genes = CNAmetTotal, BP = CNAmetBP, CC = CNAmetCC, MF = CNAmetMF,
                                                                       Pathway = CNAmetPathway) # Overlap with CCGL
COAD_FunctionalAnalysis[1, c(5, 8, 11, 14, 17)] <- EstimateCGCOverlap(Genes = CNAmetTotal, BP = CNAmetBP, CC = CNAmetCC, MF = CNAmetMF,
                                                                      Pathway = CNAmetPathway) # Overlap with CGC

## iGC
# Reading the file
iGCTotal <- read.delim("/home/anita/Benchmarking/two_omics/ColonCancerCompleteDataAnalysis/iGC/iGC_UsingOncodriveCISInput/iGC_COAD_3000Genes_OncodriveInput.txt", header = T, sep = "\t")
...
COAD_FunctionalAnalysis[2, c(3, 6, 9, 12, 15)] <- c(length(iGCTotal), nrow(iGCBP), nrow(iGCCC), nrow(iGCMF), length(iGCPathway)) # Significant genes
COAD_FunctionalAnalysis[2, c(4, 7, 10, 13, 16)] <- EstimateCCGLOverlap(Genes = iGCTotal, BP = iGCBP, CC = iGCCC, MF = iGCMF,
                                                                       Pathway = iGCPathway) # Overlap with CCGL
COAD_FunctionalAnalysis[2, c(5, 8, 11, 14, 17)] <- EstimateCGCOverlap(Genes = iGCTotal, BP = iGCBP, CC = iGCCC, MF = iGCMF,
                                                                      Pathway = iGCPathway) # Overlap with CGC


## PLRS 
# Reading the file
plrsTotal <- read.delim("/home/anita/Benchmarking/two_omics/ColonCancerCompleteDataAnalysis/plrs/PLRS_UsingOncodriveCISInput/PLRS_COAD_3000Genes_OncodriveInput.txt", header = T, sep = "\t")
...
COAD_FunctionalAnalysis[3, c(3, 6, 9, 12, 15)] <- c(length(plrsTotal), nrow(plrsBP), nrow(plrsCC), nrow(plrsMF), length(plrsPathway)) # Significant genes
COAD_FunctionalAnalysis[3, c(4, 7, 10, 13, 16)] <- EstimateCCGLOverlap(Genes = plrsTotal, BP = plrsBP, CC = plrsCC, MF = plrsMF,
                                                                       Pathway = plrsPathway) # Overlap with CCGL
COAD_FunctionalAnalysis[3, c(5, 8, 11, 14, 17)] <- EstimateCGCOverlap(Genes = plrsTotal, BP = plrsBP, CC = plrsCC, MF = plrsMF,
                                                                      Pathway = plrsPathway) # Overlap with CGC

## Oncodrive-CIS 
# Reading the file
OncoTotal <- read.delim("/home/anita/Benchmarking/two_omics/ColonCancerCompleteDataAnalysis/OncodriveCIS/Oncodrive_UsingOncodriveCISInput/Oncodrive_COAD_3000Genes_OncodriveInput.txt", header = T, sep = "\t")
...
COAD_FunctionalAnalysis[4, c(3, 6, 9, 12, 15)] <- c(length(OncoTotal), nrow(OncoBP), nrow(OncoCC), nrow(OncoMF), length(OncoPathway)) # Significant genes
COAD_FunctionalAnalysis[4, c(4, 7, 10, 13, 16)] <- EstimateCCGLOverlap(Genes = OncoTotal, BP = OncoBP, CC = OncoCC, MF = OncoMF,
                                                                       Pathway = OncoPathway) # Overlap with CCGL
COAD_FunctionalAnalysis[4, c(5, 8, 11, 14, 17)] <- EstimateCGCOverlap(Genes = OncoTotal, BP = OncoBP, CC = OncoCC, MF = OncoMF,
...
## CNAmet - Methyl
# Reading the file
CNAmetMethylTotal <- read.delim("/home/anita/Benchmarking/two_omics/ColonCancerCompleteDataAnalysis/CNAmetMethyl/CNAmetMethyl_COAD_3000Genes.txt",
                                header = T, sep = "\t") 
...
COAD_FunctionalAnalysis[5, c(3, 6, 9, 12, 15)] <- c(length(CNAmetMethylTotal), nrow(CNAmetMethylBP), nrow(CNAmetMethylCC), nrow(CNAmetMethylMF), length(CNAmetMethylPathway)) # Significant genes
COAD_FunctionalAnalysis[5, c(4, 7, 10, 13, 16)] <- EstimateCCGLOverlap(Genes = CNAmetMethylTotal, BP = CNAmetMethylBP, CC = CNAmetMethylCC, MF = CNAmetMethylMF,
                                                                       Pathway = CNAmetMethylPathway) # Overlap with CCGL
COAD_FunctionalAnalysis[5, c(5, 8, 11, 14, 17)] <- EstimateCGCOverlap(Genes = CNAmetMethylTotal, BP = CNAmetMethylBP, CC = CNAmetMethylCC, MF = CNAmetMethylMF,
                                                                      Pathway = CNAmetMethylPathway) # Overlap with CGC


## MethylMix
# Reading the file
MethylMixTotal <- read.delim("/home/anita/Benchmarking/two_omics/ColonCancerCompleteDataAnalysis/MethylMix/MethylMix_COAD_3000Genes.txt",
                             header = T, sep = "\t") 
...
COAD_FunctionalAnalysis[6, c(3, 6, 9, 12, 15)] <- c(length(MethylMixTotal), nrow(MethylMixBP), nrow(MethylMixCC), nrow(MethylMixMF), length(MethylMixPathway)) # Significant genes
COAD_FunctionalAnalysis[6, c(4, 7, 10, 13, 16)] <- EstimateCCGLOverlap(Genes = MethylMixTotal, BP = MethylMixBP, CC = MethylMixCC, MF = MethylMixMF,
                                                                       Pathway = MethylMixPathway) # Overlap with CCGL
COAD_FunctionalAnalysis[6, c(5, 8, 11, 14, 17)] <- EstimateCGCOverlap(Genes = MethylMixTotal, BP = MethylMixBP, CC = MethylMixCC, MF = MethylMixMF,
...
write.table(x = COAD_FunctionalAnalysis, file = "COAD_FunctionalAnalysis_withCGC_OncodriveInput_3000Genes_RPackages.txt", sep = "\t", quote = F, row.names = FALSE)
COAD_FunctionalAnalysis2 <- COAD_FunctionalAnalysis[, c(1,2,3,4,6,7,9,10,12,13,15,16)] # extracting information relavent only to CCGL
write.table(x = COAD_FunctionalAnalysis2, file = "COAD_FunctionalAnalysis_OncodriveInput_3000Genes_RPackages.txt", sep = "\t", quote = F, row.names = FALSE)
save.image(file = "COAD_FunctionalAnalysis_Results_OncodriveInput_3000Genes_RPackages.RData")
#######################################
library(venn)
## Venn Plots
setwd("/home/anita/Benchmarking/two_omics/ColonCancerCompleteDataAnalysis/Venns_OncodriveInput/")
load(file = "COAD_FunctionalAnalysis_Results_OncodriveInput_3000Genes_RPackages.RData")
...
########### Methylation plots
# Gene Plot
tiff("COAD_OncodriveInput_GenesVenn_Meth.tiff", width = 952, height = 952, res = 300)  
venn(x = list(CNAmet = CNAmetMethylTotal, MethylMix = MethylMixTotal, CCGL = cBioTotal),
     zcolor = "style", cexil = 0.6, cexsn = 0.6, opacity = 0.3)
dev.off()
# BP plot
tiff("COAD_OncodriveInput_BPVenn_Meth.tiff", width = 952, height = 952, res = 300)  
venn(x = list(CNAmet = CNAmetMethylBP$ID, MethylMix = MethylMixBP$ID,CCGL = cBioBP$ID),
     zcolor = "style", cexil = 0.6, cexsn = 0.6, opacity = 0.3)
dev.off()
# CC plot
tiff("COAD_OncodriveInput_CCVenn_Meth.tiff", width = 952, height = 952, res = 300)  
venn(x = list(CNAmet = CNAmetMethylCC$ID, MethylMix = MethylMixCC$ID, CCGL = cBioCC$ID),
     zcolor = "style", cexil = 0.6, cexsn = 0.6, opacity = 0.3)
dev.off()
# MF plot
tiff("COAD_OncodriveInput_MFVenn_Meth.tiff", width = 952, height = 952, res = 300)  
venn(x = list(CNAmet = CNAmetMethylMF$ID, MethylMix = MethylMixMF$ID, CCGL = cBioMF$ID),
     zcolor = "style", cexil = 0.6, cexsn = 0.6, opacity = 0.3)
dev.off()
# Pathway plot
tiff("COAD_OncodriveInput_pathwayVenn_Meth.tiff", width = 952, height = 952, res = 300)  
