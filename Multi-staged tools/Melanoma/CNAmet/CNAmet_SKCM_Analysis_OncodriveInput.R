# Analysis of Melanoma dataset using CNAmet using OncodriveCIS input
library(CNAmet)
library(tictoc)
rm(list=ls())
...
# saving results
setwd("/home/anita/Benchmarking/two_omics/MelanomaCompleteDataAnalysis/CNAmet/CNAmet_UsingOncodriveCISInput/")
write.table(ampResults, file = "CNAmet_SKCM_GainDriven_Genes_OncodriveInput.tsv", row.names = F, sep = "\t", quote = F)
write.table(delResults, file = "CNAmet_SKCM_LossDriven_Genes_OncodriveInput.tsv", row.names = F, sep = "\t", quote = F)
