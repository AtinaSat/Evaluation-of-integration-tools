# Analysis if Melanoma cancer dataset using CNAmet - methylation and gene expression integration
library(CNAmet)
library(tictoc)
rm(list=ls())
...
ge.exp <- as.matrix(skcm.ge, rownames.force = T)
...
# Hypomethylation: 2056.478 sec elapsed
# > tic("Hypermethylation")
# > hyperResults <- CNAmet(exprMatrix = ge.exp, cghMatrix = NULL, methylMatrix = me.hyper, perms = 1000, na.limit = 0.1, gainData = FALSE,
#                          +                        favorSynergetic = TRUE, strictChecks = FALSE, strictLim = 0.05)
# > toc()
# Hypermethylation: 1615.075 sec elapsed
# > toc()
# Total: 3671.559 sec elapsed
...
# saving results
setwd("/home/anita/Benchmarking/two_omics/MelanomaCompleteDataAnalysis/CNAmetMethyl/")
write.table(hypoResults, file = "CNAmetMethyl_SKCM_HypoDriven_Genes.tsv", row.names = F, sep = "\t", quote = F)
write.table(hyperResults, file = "CNAmetMethyl_SKCM_HyperDriven_Genes.tsv", row.names = F, sep = "\t", quote = F)
